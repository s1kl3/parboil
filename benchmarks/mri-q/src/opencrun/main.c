...
static void
setupMemoryCPU(int num, int size, cl_mem* dev_ptr, float* host_ptr,clPrmtr* clPrm)
{
  cl_int clStatus;
...
static void
cleanupMemoryCPU(int num, int size, cl_mem* dev_ptr, float* host_ptr, clPrmtr* clPrm)
{
  cl_int clStatus;
...
main (int argc, char *argv[]) {
  ...
  /* CPU section 1 (precompute PhiMag) */
  {
    clPrm.clKernel = clCreateKernel(clProgram,"ComputePhiMag_CPU",&clStatus);
    ...
    setupMemoryCPU(numK,sizeof(float),&phiR_d,phiR,&clPrm);
    setupMemoryCPU(numK,sizeof(float),&phiI_d,phiI,&clPrm);
    ...
    computePhiMag_CPU(numK, phiR_d, phiI_d, phiMag_d, &clPrm);
    ...
    cleanupMemoryCPU(numK,sizeof(float),&phiMag_d,phiMag,&clPrm);
  ...
  /* CPU section 2 */
  {
    clPrm.clKernel = clCreateKernel(clProgram,"ComputeQ_CPU",&clStatus);
    CHECK_ERROR("clCreateKernel")
    ...
    setupMemoryCPU(numX,sizeof(float),&x_d,x,&clPrm);
    setupMemoryCPU(numX,sizeof(float),&y_d,y,&clPrm);
    setupMemoryCPU(numX,sizeof(float),&z_d,z,&clPrm);
    ...
    computeQ_CPU(numK, numX, x_d, y_d, z_d, kVals, Qr_d, Qi_d, &clPrm);
    ...
    CHECK_ERROR("clReleaseMemObject")
    cleanupMemoryCPU(numX,sizeof(float),&Qr_d,Qr,&clPrm);
    cleanupMemoryCPU(numX,sizeof(float),&Qi_d,Qi,&clPrm);
  }
