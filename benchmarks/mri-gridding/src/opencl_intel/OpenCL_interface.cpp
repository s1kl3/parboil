...
/***********************************************************************
 * CUDA_interface is the main function for CPU execution. This
 * implementation uses compact binning to distribute input elements
 * into unit-cubed sized bins. The bins are then visited by CPU
 ...
 * is offloaded to the CPU to be computed in parallel with the CPU
 * gridding.
 ***********************************************************************/
void OpenCL_interface (
  ...
  int dims[3] = {8,4,2}; //size of a gridding block on the CPU
  ...
  const char *source_path = "src/opencl_intel/CPU_kernels.cl";
  char *source;
  ...
  */
  ...
  gridding_CPU = clCreateKernel(gpu_kernels, "gridding_CPU", &ciErrNum);
  OCL_ERRCK_VAR(ciErrNum);
  ...
  /* STEP 5: Perform the binning on the CPU. The results are computed in a gather fashion
  ...
  OCL_ERRCK_RETVAL( clSetKernelArg(gridding_CPU, 0, sizeof(cl_mem), (void *)&sortedSample_d) );
  OCL_ERRCK_RETVAL( clSetKernelArg(gridding_CPU, 1, sizeof(cl_mem), (void *)&binStartAddr_d) );
  OCL_ERRCK_RETVAL( clSetKernelArg(gridding_CPU, 2, sizeof(cl_mem), (void *)&gridData_d) );
  OCL_ERRCK_RETVAL( clSetKernelArg(gridding_CPU, 3, sizeof(cl_mem), (void *)&sampleDensity_d) );
  OCL_ERRCK_RETVAL( clSetKernelArg(gridding_CPU, 4, sizeof(float), &beta) );
  
  OCL_ERRCK_RETVAL ( clEnqueueNDRangeKernel(clCommandQueue, gridding_CPU, 3, 0,
                            grid2, block2, 0, 0, 0) );
   ...
   * and adding those to the CPU results.
   */
