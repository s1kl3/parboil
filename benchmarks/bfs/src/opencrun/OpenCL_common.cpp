    ...
    for (int id = 0; (id < numDevices) && needDevice ; ++id) {
      ...
      if (reqDeviceType != NULL) {
        ...
        if (*reqDeviceType != CL_DEVICE_TYPE_ALL) {
          if (*reqDeviceType != (clDeviceType ^ CL_DEVICE_TYPE_DEFAULT)) {
            canSatisfy = false;
          }
        }
      }
      ...
      if (canSatisfy) {
        ...
        *platform = clPlatform;
        ...
        fprintf(stderr, "Chose Device Type: %s\n",
          ((clDeviceType ^ CL_DEVICE_TYPE_DEFAULT) == CL_DEVICE_TYPE_CPU) ? "CPU" : ((clDeviceType ^ CL_DEVICE_TYPE_DEFAULT) == CL_DEVICE_TYPE_GPU) ? "GPU" : "other"
          );
