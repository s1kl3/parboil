...
LANG_CFLAGS=-I$(PARBOIL_ROOT)/common/include -I$(OPENCRUN_PATH)/include
LANG_CXXFLAGS=$(LANG_CFLAGS)
LANG_LDFLAGS=-lOpenCRun -L$(OPENCRUN_LIB_PATH)
...
run:
	@echo "Resolving OpenCRun library..."
	@$(shell echo $(RUNTIME_ENV)) LD_LIBRARY_PATH=$(OPENCRUN_LIB_PATH) ldd $(BIN) | grep OpenCRun
	@$(shell echo $(RUNTIME_ENV)) LD_LIBRARY_PATH=$(OPENCRUN_LIB_PATH) $(BIN) $(ARGS)

debug:
	@echo "Resolving OpenCRun library..."
	@$(shell echo $(RUNTIME_ENV)) LD_LIBRARY_PATH=$(OPENCRUN_LIB_PATH) ldd $(BIN) | grep OpenCRun
	@$(shell echo $(RUNTIME_ENV)) LD_LIBRARY_PATH=$(OPENCRUN_LIB_PATH) $(DEBUGGER) --args $(BIN) $(ARGS)

clean :
	rm -f $(BUILDDIR)/*
	if [ -d $(BUILDDIR) ]; then rmdir $(BUILDDIR); fi

$(BIN) : $(OBJS) $(BUILDDIR)/parboil_opencrun.o
	$(CXX) $^ -o $@ $(LDFLAGS)
...
$(BUILDDIR)/parboil_opencrun.o : $(PARBOIL_ROOT)/common/src/parboil_opencrun.c
	$(CC) $(CFLAGS) -c $< -o $@
...
no_opencrun:
	@echo "OPENCRUN_PATH is not set. Open $(PARBOIL_ROOT)/common/Makefile.conf to set default value."
